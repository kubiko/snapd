...
var maybeApplyPreseededData = func(st *state.State, model *asserts.Model, ubuntuSeedDir, sysLabel, writableDir string) (preseeded bool, err error) {
	...
	preseedAs, err := installLogic.ReadPreseedAssertion(assertstate.TemporaryDB(st), model, ubuntuSeedDir, sysLabel)
	if err != nil {
		return false, err
	}
	if err = installLogic.MaybeApplyPreseededData(model, preseedAs, preseedArtifact, ubuntuSeedDir, sysLabel, writableDir); err != nil {
		return false, err
	} else {
		return true, nil
	}
