...
//
// A kernel revision that is not referenced by any of the environment variables
// the bootloader resolves against the matrix - snap_kernel, snap_try_kernel and,
// for recovery environments, snapd_recovery_system and try_recovery_system - is
// not used to boot the device. The bootloader never searches the matrix for it,
// so no boot image partition it points at can be selected, and the redundant
...
func (l *lk) repairDuplicateKernelBootPartitions(env *lkenv.Env) error {
	...
	for kernel, bootPartitions := range duplicates {
		...
		referenced := env.IsBootValueReferenced(kernel)
