...
// bootValueVariables are the environment variables through which the bootloader
// is directed at a boot image matrix value. Any value named by one of them is
// referenced for booting, everything else in the matrix is not. Variables that
// do not exist in a given lkenv version read back as empty and are simply not
// considered.
var bootValueVariables = []string{
	// the kernel matrix of v1 and v2 run environments
	"snap_kernel",
	"snap_try_kernel",
	// the recovery system matrix of v2 recovery environments
	"snapd_recovery_system",
	"try_recovery_system",
}

// IsBootValueReferenced returns whether the given boot image matrix value is
// named by any of the environment variables the bootloader resolves against the
// matrix, i.e. whether it is referenced for booting the device.
//
// A value that is not referenced by any of them is not used to boot: the
// bootloader never searches the matrix for it, so the boot image partition it
// points at cannot be selected and its reference can be dropped without
// affecting how the device boots.
func (l *Env) IsBootValueReferenced(value string) bool {
	if value == "" {
		return false
	}

	for _, key := range bootValueVariables {
		if l.variant.get(key) == value {
			return true
		}
	}
