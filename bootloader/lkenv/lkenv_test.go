...
func (l *lkenvTestSuite) TestIsBootValueReferenced(c *C) {
	tt := []struct {
		version lkenv.Version
		// the environment variables the bootloader resolves against the boot
		// image matrix for this lkenv version
		keys []string
		// variables that exist in other versions only and must be ignored here
		otherKeys []string
		comment   string
	}{
		{
	...
	for _, t := range tt {
		...
		// nothing is referenced in a pristine environment, and the empty value
		// is never referenced even though every variable reads back empty
		c.Check(env.IsBootValueReferenced(""), Equals, false, comment)
		c.Check(env.IsBootValueReferenced("kernel-1"), Equals, false, comment)

		for _, key := range t.keys {
			env.Set(key, "kernel-1")
			c.Check(env.IsBootValueReferenced("kernel-1"), Equals, true,
				Commentf("%s: %s", t.comment, key))
			// an unrelated value is still not referenced
			c.Check(env.IsBootValueReferenced("kernel-2"), Equals, false,
				Commentf("%s: %s", t.comment, key))
			// and the empty value stays unreferenced
			c.Check(env.IsBootValueReferenced(""), Equals, false,
				Commentf("%s: %s", t.comment, key))
			env.Set(key, "")
			c.Check(env.IsBootValueReferenced("kernel-1"), Equals, false,
				Commentf("%s: %s", t.comment, key))
		}

		// variables belonging to other lkenv versions are not settable here, so
		// they can never make a value look referenced
